...
def print_search_result(result:Tuple[int, int, Optional[List[Point]]]) -> None:
    print("Visited:",result[0])
    if result[2]:
        print("Best path cost:",result[1])
        print(pretty_print_path(result[2]))
    else:
        print("No path found")
...
def breadth_first(terrain:Map, start:Point, goal:Point) -> Tuple[int, int, Optional[List[Point]]]:
    ...
    best_costs: Dict[int, Point] = {start:(0, start)}
    visit_count = 0
    while open_list:
...
# Example
terrain = load_map('terrain.txt')
print_search_result(breadth_first(terrain, (0, 0), (10, 0)))
print_search_result(breadth_first(terrain, (2, 3), (7, 0)))
print_search_result(breadth_first(terrain, (5, 5), (0, 1)))
print_search_result(breadth_first(terrain, (0, 0), (11, 10)))
#+END_SRC
...
16. Do you think this strategy would give optimal paths if we always picked the first path we found?  Why or why not?
17. Does best-first search need to return the first found path or should it wait until examining all paths, as in breadth-first search?  Why?
18. Should best-first search avoid re-expanding nodes with higher costs, as we did for the earlier algorithms?
...
19. Try best-first search on the examples from before.  How does it differ in terms of visited nodes?  In terms of found paths?
20. What is the main difference between best-first search and your implementation of Dijkstra's algorithm from before?
...
21. Can you think of a way to combine the priority information from Dijkstra's algorithm (cost to get there) and from best-first search (estimated cost to reach the goal)?  What is the priority in the search state, and what do you track in =best_costs=?
...
22. Compare your implementation of A* to your previous implementations in terms of nodes visited and paths found.
23. Is the first solution found by A* in this domain guaranteed to be cost-optimal?
24. What if we replace calls to the heuristic function (used as part of determining priority) with the constant 0?  Does this behave like a different algorithm in terms of nodes visited and path found?
25. How about if we replace the uses of the cost so far in the priority calculations with 0?  Does this behave like a different algorithm in terms of nodes visited and path found?
26. When a heuristic /overestimates/ the distance to the goal, it is called /inadmissible/.  We could make our heuristic inadmissible just by multiplying its output by some factor, say 10, before using its results in the priority calculation.  How does this affect nodes visited and path found for the examples given above?
