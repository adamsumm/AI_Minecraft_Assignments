...
2. [@2] How do we need to change our decision variables to account for these improvements?
...
3. [@3] If we made this alteration, how would the constraints relating the current inventory to the previous inventory need to change?
...
6. [@6] In a formal notation, what will the constraints on =Requires= terms for recipes look like now?
...
7. [@7] Try this algorithm with different combinations of initial and goal states.  How does performance vary with the number of distinct items, the number of items total, and the length of the dependency chain involved in creating the items?
8. What's the largest crafting problem you can think of that your algorithm can solve within 20 seconds?
